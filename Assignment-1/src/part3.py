...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, np.prod(list(x.size()[1:]) + [1]))
...
class NNModel:
    ...
    def view_batch(self):
        ...
        """
        img, lbl = next(iter(self.trainloader))

        img = torch.reshape(img, (8, 8, 28, 28))
        img = img.permute(0, 2, 1, 3)
        img = torch.reshape(img, (224, 224))

        lbl = torch.reshape(lbl, (8, 8))
